// Manual Store Activation Script
// This will activate each test store using your OAuth token
...
console.log('Manual Store Activation Commands:');
console.log('==================================');

testStores.forEach((store, index) => {
    const payload = JSON.stringify({
        integratorBrandId: store.integratorBrandId,
        integratorStoreId: store.integratorStoreId,
    ...
    console.log(`\n${index + 1}. ${store.name}`);
    console.log(`curl -X POST "https://func-burger-api-lf6kch3t2wm3e.azurewebsites.net/api/uber/stores/${store.storeId}/activate?userId=c31ba0dc9389a994ced0ecec9837f611" \\`);
